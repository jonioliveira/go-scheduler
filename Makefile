...
export REPOSITORY?=jonioliveira/go-scheduler
export VERSION?=dev-latest

GO_LDFLAGS=-ldflags "-X git.daimler.com/CCP/kns-helm-api/internal/metadata.version=$$VERSION"
...
DOCKER_COMPOSE=docker-compose -f deployments/docker/compose.yaml -p kns-helm-api
...
shell-go-build:
	go build ${GO_LDFLAGS} -o bin/kns-helm-api cmd/kns-helm-api/main.go
...
.PHONY: shell-go-run
shell-go-run:
	go run  ${GO_LDFLAGS} cmd/kns-helm-api/main.go $(filter-out $@,$(MAKECMDGOALS))
